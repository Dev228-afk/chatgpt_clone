...
document.body.style.backgroundImage = "url('/client/img/clipart-sharing-pencil-4.png')";
...
function translateToGujarati(text) {
  ...
  var apiKey = "AIzaSyB9bkcQLywCJxTUgH4F-w95r1ZBf4VgWz8";
  ...
  xhr.onload = function () {
    if (xhr.status === 200) {
      ...
      translation = JSON.parse(xhr.responseText).translatedText[0];
      console.log("The translation is: " + translation);
    } else {
      // The translation failed.
      console.log("The translation failed with status code: " + xhr.status);
    }
  };
  return translation;
}
...
const ask_gpt = async (message) => {
  try {
    ...
    // var trans= waitforload(getscript('http://www.google.com/jsapi'));
    while (true) {
      ...
      if (
        chunk.includes(
          `<form id="challenge-form" action="${url_prefix}/backend-api/v2/conversation?`
        )
      ) {
        chunk = `cloudflare token expired, please refresh the page.`;
      }

      text += chunk; // final response generated by gpt
    ...
    trans_text = translateToGujarati(text);
    add_message(window.conversation_id, "user", message);
    add_message(window.conversation_id, "assistant", trans_text);
